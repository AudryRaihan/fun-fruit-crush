...
public class Board : MonoBehaviour {
    ...
    private void Shuffler()
    {
        ...
        // Kumpulkan semua dot dalam list
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (allDots[i, j] != null)
                {
                    ...
                    //Debug.Log(allDots[i, j]);
                }
            }
        }

        // Fisher-Yates shuffle
        for (int i = newBoard.Count - 1; i > 0; i--)
        {
            int j = Random.Range(0, i + 1);
            (newBoard[i], newBoard[j]) = (newBoard[j], newBoard[i]);
        }

        // Reset papan dengan hasil shuffle
        int index = 0;
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (!blankSpaces[i, j] && !concreteTiles[i, j] && !slimeTiles[i, j])
                {
                    GameObject candidateDot = null;
                    int searchIndex = index;
                    int maxIterations = 0;

                    // Cari posisi dari dot yang tidak menyebabkan match
                    while (searchIndex < newBoard.Count && maxIterations < 100)
                    {
                        candidateDot = newBoard[searchIndex];
                        if (!MatchesAt(i, j, candidateDot))
                        {
                            // Pastikan kita tidak pakai dot yang sama dua kali
                            newBoard.RemoveAt(searchIndex);
                            break;
                        }
                        searchIndex++;
                        maxIterations++;
                    ...
                    if (candidateDot != null)
                    {
                        Dot dot = candidateDot.GetComponent<Dot>();
                        dot.column = i;
                        dot.row = j;
                        candidateDot.transform.position = new Vector2(i, j);
                        candidateDot.name = "( " + i + ", " + j + " )";

                        allDots[i, j] = candidateDot;
                    }
                    else
                    {
                        Debug.LogWarning($"Tidak ada dot valid untuk ({i}, {j}) setelah 100 iterasi.");
                    }
                }
            }
        }
        ...
        if(allDots[column, row].GetComponent<Dot>().isMatched){
            ...
            if (soundManager != null)
            {
                if (PlayerPrefs.HasKey("SfxMusic"))
                {
                    ...
                    {
                        soundManager.PlaySfxMusic();
                    }
                    else if (PlayerPrefs.GetInt("SfxMusic") == 0)
                    {
                        soundManager.MuteSfxMusic();
                    }
                    else
                    {
                        Debug.Log("nilainya bukan 0 atau 1" + PlayerPrefs.GetInt("SfxMusic"));
                    }
                }
